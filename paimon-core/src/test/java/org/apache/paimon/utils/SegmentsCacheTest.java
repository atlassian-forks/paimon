...
public class SegmentsCacheTest {
    ...
    /** Must mirror the private constants in {@link SegmentsCache}. */
    private static final long OBJECT_MEMORY_SIZE = 1000;
    ...
    @Test
    public void testWeighIncludesPerSegmentOverheadForSingleSegment() {
        SegmentsCache<String> cache =
                new SegmentsCache<>(1024, MemorySize.ofMebiBytes(64), Long.MAX_VALUE, null, false);
        cache.put("k", new SingleSegments(MemorySegment.allocateHeapMemory(1024), 1024));
        assertThat(cache.totalCacheBytes())
                .isEqualTo(OBJECT_MEMORY_SIZE + PER_SEGMENT_OVERHEAD + 1024L);
    }

    @Test
    public void testWeighScalesWithSegmentCountForMultiSegments() {
        SegmentsCache<String> cache =
                new SegmentsCache<>(1024, MemorySize.ofMebiBytes(64), Long.MAX_VALUE, null, false);
        int n = 4;
        ArrayList<MemorySegment> segments = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            segments.add(MemorySegment.allocateHeapMemory(1024));
        }
        cache.put("k", new MultiSegments(segments, 1024));
        long expected = OBJECT_MEMORY_SIZE + (long) n * PER_SEGMENT_OVERHEAD + (long) n * 1024;
        assertThat(cache.totalCacheBytes()).isEqualTo(expected);
    }
