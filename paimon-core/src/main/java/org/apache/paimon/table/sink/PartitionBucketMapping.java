...
public class PartitionBucketMapping implements Serializable {
     ...
     * string. Built once at load time from the table's partition schema so that {@link
     * #resolveNumBuckets(BinaryRow)} doesn't have to repeatedly convert types per record. May be
     * {@code null} for legacy code paths that constructed this object without schema info, in which
     * case logging falls back to {@link BinaryRow#toString()}.
     */
     ...
     * once per partition, not on every record. Marked {@code transient} so it does not get shipped
     * with the serialized state.
     */
    private transient Set<BinaryRow> firstResolutionLogged;

    /**
     * Functional interface used to render a partition {@link BinaryRow} as a human-readable string.
     * Holding a tiny wrapper here (rather than the full converter + key list) lets unit tests stub
     * the rendering without depending on {@code RowDataToObjectArrayConverter}.
     */
    @FunctionalInterface
    ...
    /**
     * Same as {@link #PartitionBucketMapping(int, Map)} but additionally accepts a {@link
     * PartitionRenderer} used to format partition values in log output. When the renderer is {@code
     * null}, logging falls back to {@link BinaryRow#toString()} (which prints {@code
     * BinaryRow@hex}).
     */
    public PartitionBucketMapping(
            ...
            if (firstResolutionLogged.add(partition.copy())) {
                ...
                if (partitionRenderer != null) {
                    try {
                        ...
                        partitionStr = "<render failed: " + t.getClass().getSimpleName() + ">";
                    }
                } else {
